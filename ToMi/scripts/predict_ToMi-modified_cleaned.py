...
def average_accuracy(args): #with csv report of mislabeled
    ...
    file1.close()
    with open(f'./{args.results_dir}/{args.dataset}-{args.model}-results.txt') as f_in:
...
def predict(args, pipe=None):
    with open(f'./{args.results_dir}/{args.dataset}-{args.model}-results.txt', "a") as f_out:
        with open(f"./{args.data_dir}/{args.dataset}.txt") as f_in:
            for i, line in enumerate(tqdm.tqdm(f_in)):
                ...
                prompt = (
                    ...
                    "Question: " + fields['question'].strip() + "Answer with one word only, without explanations. Answer:"
                )
                if args.model == "llama-3.1-8B-instruct":
                    fields['prediction'] = llama_finalanswer_request(prompt, i, pipe)
                else:
                    fields['prediction'] = open_ai_finalanswer_request(prompt, i, 0, args.model)
...
def llama_finalanswer_request(prompt, i, llama_pipeline):
    try:
        messages = [
        ...
        # prediction = prediction.lstrip('the ').strip()
...
def parse_args():
    parser = argparse.ArgumentParser(description='Run ToMi predictions')
...
def main():
    ...
    with open(f'./{args.results_dir}/ToMi_{args.dataset}-{args.model}-mislabeled.csv', 'w', newline='') as csvfile:
        fieldnames = ['context', 'question', 'label', 'prediction', 'prompt', 'question type', 'belief']
