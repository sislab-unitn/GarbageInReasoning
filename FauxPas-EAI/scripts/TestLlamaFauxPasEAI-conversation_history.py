...
if __name__ == '__main__':
    ...
    for i in fp_parser.tests:
        ...
        for j in range(number_of_samples):
            ...
            s = response[len(full_prompt):].strip().replace("\n", " ")
        ...
        for j in range(number_of_samples):
            ...
            response = pipe(full_prompt)[0]['generated_text']
            s = response[len(full_prompt):].strip()
    ...
    with open(f"fp_conv_hist_{model_name.split('/')[-1]}_s-{number_of_samples}_t-{temperature}_acc-{acc}.txt", 'w') as f_out:
        f_out.write(out_str)
    with open(f"fp_conv_hist_{model_name.split('/')[-1]}_s-{number_of_samples}_predictions.txt", 'w') as f_out:
        f_out.write("\n".join(pred_answers)) 